...
monte3=10
f = open('Results/Results_CMI.txt','w')
...
for j in range(n_Reg):
    ...
    for i in range(monte3):
        ...
        UNI_test3.append(H.UNI(np.array(z_test), np.array(y_test),np.array(c_test),dm1,dm2,dm3))
        UNI_test_pred3.append(H.UNI(np.array(z_test), np.array(y_pred>0.5),np.array(c_test),dm1,dm2,dm3))
    ...
    f.write('AUC:{}\n'.format(np.mean(np.array(AUC3))))
    f.write('AUC(Median):{}({})\n'.format(np.median(np.array(AUC3)),np.std(np.array(AUC3))))
    f.write('Accuracy:{}\n'.format(np.mean(np.array(Acc3))))
    f.write('Accuracy(Median):{} ({})\n'.format(np.median(np.array(Acc3)),np.std(np.array(Acc3))))
    f.write('MI with True Label:{}\n'.format(np.mean(np.array(MI_test3))))
    f.write('MI with Output:{}\n'.format(np.mean(np.array(MI_test_pred3))))
    f.write('CMI with True Label:{}\n'.format(np.mean(np.array(CMI_test3))))
    f.write('CMI with Output:{} \n'.format(np.mean(np.array(CMI_test_pred3))))
    f.write('UNI with True Label:{}\n'.format(np.mean(np.array(UNI_test3))))
    f.write('UNI with Output:{} \n'.format(np.mean(np.array(UNI_test_pred3))))
